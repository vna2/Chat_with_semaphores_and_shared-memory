...
int main(int argc, char const *argv[]){
    ...
    if(resend_message(ENC_CHAN_shared_mem_key_file,ENC_CHAN_shared_mem_size_file)==1){
        ...
        #if DEBUG >= 1
            printf("~ ENC %d releasing recend %d\n", getpid(),sem_ENC_p1_id);
        #endif
        semaphore_signal(sem_ENC_p1_id);
        ...
        #endif
        semaphore_signal(sem_p_resend_p1_id);
        #if DEBUG >= 1
            printf("~ENC %d waiting recend CHAN ,%d\n", getpid(),sem_ENC_resend_p1_id);
        #endif
